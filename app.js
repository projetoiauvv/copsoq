...
const QUESTION_TEXTS = [
  ...
  "O seu trabalho apresenta objetivos claros?",
  "Sabe exatamente quais as suas responsabilidades?",
  "Sabe exatamente o que é esperado de si?",
  ...
  "Ansioso?",
  "Triste?",
  "Falta de interesse por coisas cotidianas?",
...
function normalizeQuestionText(value) {
  ...
  normalized = normalized
    .replace(/\bobjectivos?\b/g, "objetivos")
    .replace(/\bexactamente\b/g, "exatamente")
    .replace(/\bafecta\b/g, "afeta")
    .replace(/\bquotidianas\b/g, "cotidianas")
    .replace(/\bplaneamento\b/g, "planejamento");
...
function parseCsv(text){
  ...
  for(let i=bestHeaderIdx+1;i<lines.length;i+=1){
    ...
    if (
      firstCell === "id" ||
      firstCell === "q1" ||
      firstCell.includes("hora de inicio") ||
      firstCell.includes("hora de início")
    ) continue;
...
async function readSpreadsheetAsCsvText(file) {
  ...
  if (fileName.endsWith(".xlsx") || fileName.endsWith(".xls")) {
    ...
    return window.XLSX.utils.sheet_to_csv(worksheet, { FS: ";", RS: "\n" });
  }
  return file.text();
}
